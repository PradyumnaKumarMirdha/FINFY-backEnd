...
def phase1(driver,stock_data):
    ...
    stock_data["performance"]["1 week"] = PerformanceSoupObj.contents[1].span.contents[1].get_text()
    stock_data["performance"]["1 month"] = PerformanceSoupObj.contents[2].span.contents[1].get_text()
    stock_data["performance"]["6 months"] = PerformanceSoupObj.contents[3].span.contents[1].get_text()
...
def phase2(driver,stock_data):
    ...
    while(keystatsIndex < len(KeystatsSoupObj.contents) - 1):
        if(keystatsIndex == 3):
            keystatsIndex += 1
            continue
        stock_data[stock_data_keys[stock_data_keysIndex]] = KeystatsSoupObj.contents[keystatsIndex].contents[1].div.get_text()
...
def phase3(driver,stock_data):
    ...
    try:
       for i in range(len(UpperAbout.contents)):                                #changed this part totally
        ...
        stock_data["data_preview"]["stock_price"] = DataPreviewSoupObj.contents[3].div.div.div.span.get_text()
        stock_data["data_preview"]["symbol_change"] = DataPreviewSoupObj.contents[3].div.div.contents[1].span.get_text()
        stock_data["data_preview"]["symbol_change_pt"] = DataPreviewSoupObj.contents[3].div.div.contents[1].contents[1].get_text()
        return stock_data
...
def StockDetail(stock_symbol):
    ...
    try:
        ...
        driver.get(URL)
        stock_data = {"performance":{"1 week":None,"1 month":None,"6 months":None},"mrkt_capita":None,"div-yield":None,"PE_ratio":None,"Net_income":None,"Revenue":None,"Share_float":None,"about":{"Sector":None,"Industry":None,"CEO":'-',"Website":None,"Headquarters":None,"Employees (FY)":None,"Founded":None,"ISIN":None,"FIGI":None,"desc":None},"data_preview":{"stock_image":None,"company":None,"stock_price":None,"symbol_change":None,"symbol_change_pt":None}}
